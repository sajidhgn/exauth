defmodule ExauthWeb.AuthController do
  ...
  case Accounts.create_user(params) do
    ...
    {:ok, _user} -> conn |> render("ack.json", %{success: true, message: "Registeration successful"})
  ...
  def login(conn, %{"username" => username, "password" => password}) do
    with %User{} = user <- Accounts.get_user_by_username(username),
         true <- Pbkdf2.verify_pass(password, user.password) do
          signer = Joken.Signer.create("HS256","Gs09dFPo9jggjPLc57dvz3O/v5RifmRuHz1jPlPOIVQQXhqLggy083Nm8egqSPwQ")
